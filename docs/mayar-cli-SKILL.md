...
<!-- ====== BATAS DOKUMEN ASLI MAYAR ====== -->
<!-- Bagian di bawah ini BUKAN bagian dari SKILL.md upstream; ditambahkan oleh proyek ini. -->

## Catatan Proyek Game Edukasi Anak (tambahan kami, bukan bagian dokumen asli)

**Sumber:** [`mayarid/mayar-cli`](https://github.com/mayarid/mayar-cli) — file `SKILL.md` di branch `main`, disalin verbatim pada **2026-07-26** (versi skill `1.2.0`). Lisensi upstream: MIT (lihat field `license` di frontmatter). Semua yang di ATAS pemisah ini adalah teks asli tanpa perubahan.

**Kenapa disimpan di repo:** Mayar.id adalah salah satu kanal penjualan yang sudah diputuskan di `CLAUDE.md`. Dokumen ini jadi rujukan saat mengerjakan **Fase 5 — Monetisasi**, supaya tidak perlu menebak-nebak nama endpoint/perintah.

**Untuk memperbarui:**

```bash
curl -fsSL https://raw.githubusercontent.com/mayarid/mayar-cli/main/SKILL.md -o docs/mayar-cli-SKILL.md
```

(lalu tempelkan kembali catatan ini di bagian bawah). Alternatif: `npx -y mayar@latest skill install --target claude` memasang skill ini ke `.claude/skills/` sehingga aktif otomatis di sesi Claude Code — **belum dilakukan**, karena akan mengubah perilaku semua sesi; putuskan saat Fase 5 dimulai.

### Poin yang paling relevan untuk kita

1. **Jangan panggil CLI dari Cloud Function.** Aturan upstream tegas: CLI untuk administrasi manual di terminal; untuk kode aplikasi, ambil spesifikasi lewat `mayar docs <topic>` lalu tulis HTTP request native (`fetch`) di `functions/`.
2. **Sandbox tersedia** — `api.mayar.club` (flag `--sandbox`), produksi `api.mayar.id`. Ujicoba alur beli → aktivasi bisa dilakukan tanpa uang sungguhan.
3. **Ada API lisensi bawaan** yang mirip persis rencana kode aktivasi kita:
   - `saas activate|deactivate|verify <licenseCode> <productId>`
   - `software verify <licenseCode> <productId>`
   - Tipe produk `saas` tersedia di `product type <ebook|course|membership|saas|event|webinar>`.
   - **Implikasi (BELUM diputuskan):** kalau ini dipakai, koleksi `activation_codes` + script generator kode batch di Fase 5 mungkin tidak perlu — Mayar yang menerbitkan & memvalidasi kode. Batas 3 perangkat kemungkinan tetap harus kita simpan sendiri di `users/{uid}/devices`. **Jangan ubah arsitektur di `CLAUDE.md` tanpa konfirmasi pemilik proyek.**
4. **Webhook** (`webhook register <url>`, `test`, `history`, `retry`) — memungkinkan Cloud Function menerima notifikasi pembayaran lunas dan membuka akses kelompok otomatis, tanpa pembeli mengetik kode. Ini alternatif alur yang perlu dibandingkan dengan alur "kode aktivasi manual" di `CLAUDE.md`.
5. **Data pembeli** (`customer create` dengan field `name`, `email`, `mobile`) — nomor WhatsApp tersedia, jadi kode aktivasi bisa dikirim via WA, bukan email saja. Lebih cocok untuk orang tua di Indonesia.

### Yang masih belum diketahui

Bentuk payload `saas verify` dan daftar event webhook **tidak ada** di `SKILL.md` maupun `README.md` upstream. Keduanya harus diambil lewat `npx -y mayar@latest docs <topic>` dengan API key asli sebelum Fase 5 dirancang final.
