...
5. **Fase 5 — Monetisasi:** Cloud Function validasi kode, script generator kode, device limit, halaman aktivasi. ✅ **KODE SELESAI** — sisanya prasyarat akun pemilik (KYC Mayar, project Firebase, Blaze). Rencana & status di **`docs/monetisasi-mayar.md`**.
...
- **Fase 5 dimulai: generator kode aktivasi (`scripts/generate-codes.mjs`)** (2026-07-28), teruji 3500 kode (verifikasi, deteksi typo, dedup lintas batch, semua jalur error CLI):
  ...
  - **Jalur rilis pertama = "jalur A" (voucher pre-generated)**: kode dibuat batch → di-upload ke Mayar sebagai stok voucher produk digital → Mayar kirim otomatis ke pembeli. Tidak butuh webhook & tidak butuh integrasi API Mayar. Webhook otomatis ("jalur B") menyusul setelah penjualan stabil. **Koreksi 2026-07-28: jalur A tetap butuh Blaze** — penukaran kode divalidasi Cloud Function, dan itu mensyaratkan billing (biaya praktis nol di skala kita).
...
- **Fase 5 (Monetisasi) — KODE SELESAI** (2026-07-28), teruji lawan Firebase Emulator (42 assertion: 29 callable + 13 rules) & alur UI di browser 380×800 (daftar akun → kode salah ditolak → kode benar → game premium terbuka, tanpa error console):
  - **Sisa pekerjaan Fase 5 murni urusan akun pemilik**: KYC Mayar, buat project Firebase, aktifkan Blaze. Begitu `.env` diisi → `firebase deploy --only functions,firestore:rules`. Rincian di `docs/monetisasi-mayar.md`.
  - **Format kode pindah ke `functions/src/activation-code.ts`** — SATU sumber kebenaran. `scripts/generate-codes.mjs` meng-import-nya (Node 22 melepas tipe sendiri, tanpa build step); file-nya harus di `functions/` karena hanya folder itu yang diunggah saat deploy. **JANGAN menyalin algoritma checksum ke tempat lain** — kode yang sudah dijual jadi tidak valid kalau dua salinan menyimpang. Kode yang dibuat sebelum refactor tetap valid (algoritma tidak berubah).
  - **Dua callable di `functions/src/index.ts`**, region **`asia-southeast2` (Jakarta)** — harus sama dengan `FUNCTIONS_REGION` di `src/auth/firebase.ts`, beda region = semua callable 404:
    - `redeemActivationCode({ code })`: cek checksum dulu (kode salah ketik ditolak tanpa query Firestore) → rate limit 10 percobaan/10 menit per akun (`redeem_attempts/{uid}`) → satu transaksi menandai `used`/`usedBy`/`usedAt` + `arrayUnion` grup ke `users/{uid}.groups`. **Idempotent untuk pemiliknya** (`alreadyOwned: true`, bukan error) karena orang tua sering menekan tombol dua kali; kode milik akun lain selalu ditolak.
    - `verifyAccess({ deviceId })`: cek akses online + daftarkan perangkat, maksimal 3. Perangkat yang sudah terdaftar cuma di-update `lastSeenAt`, jadi HP yang sama tak pernah memakan kuota.
  - **`firestore.rules` diperketat**: `users/{uid}/devices` jadi **read-only untuk client** (dulu boleh tulis — user tinggal menghapus entri lama untuk melewati batas 3 perangkat), dan `redeem_attempts` tertutup total (kalau bisa ditulis client, rate limit tinggal di-reset sendiri).
  - **Gerbang akses game** (`src/portal/GamePage.tsx`): game premium memanggil `refreshAccess()` **saat diluncurkan**, bukan cuma saat login. Chunk config game premium baru di-import SETELAH akses diberikan, jadi data level berbayar tak pernah sampai ke browser yang tak berhak. Kalau pengecekan gagal (internet putus), akses bertahan di hasil cek terakhir — gagal-tertutup akan mengunci pembeli yang sah.
  - **Identitas perangkat = id acak di localStorage** (`src/auth/device.ts`), sengaja BUKAN browser fingerprint: jujur, tanpa tracking. Hapus data browser = terhitung perangkat baru; itu diterima, karena batas ini untuk mencegah akun dibagi-bagi, bukan menghukum keluarga biasa.
  - **Pesan error selalu Bahasa Indonesia yang ramah orang tua** (`src/auth/access.ts` menerjemahkan kode error Firebase). Jangan pernah menampilkan `functions/...` mentah ke layar.
  - **Menguji tanpa project Firebase**: emulator menerima project id berawalan `demo-`, jadi tak perlu akun/billing. `npm run emulators` lalu `npm run test:emulator`. Nilai `.env` demo ada di `.env.example`.
