...
- **Tambah Tangkas dirombak: persamaan tampil di layar + 7 soal per sesi + kolam varian** (2026-08-01, permintaan pemilik dari tangkapan layar "Berapa 4 tambah 4?" yang tengahnya kosong), teruji headless 360×640, 380×800 & 820×1180 (12 sesi × 7 soal dimainkan sampai "Selamat!", 59 soal berbeda terlihat, tanpa scroll horizontal/vertikal & tanpa error console):
  ...
  - **Acuan kurikulum matematika SD Kelas 1 & 2: `docs/kurikulum-sd1-2.md`** (dari pemilik, 2026-09-02) — daftar bab per semester + bab mana yang sudah/belum punya game. Tiap game matematika baru harus bisa ditunjuk ke salah satu bab di situ. **Catatan penting: kurikulum kelas 2 minta bilangan sampai 50 lalu 100, jadi bentrok dengan batas 30 di bawah — jangan naikkan batasnya tanpa keputusan pemilik.**
...
- **Jam Pintar: muka jam digambar sendiri, LENGKAP DENGAN ANGKA 1–12** (2026-08-01, keluhan pemilik dari tangkapan layar "jamnya gaada angkanya"), teruji headless 360×640, 380×800 & 820×1180 (8 slot dimainkan sampai "Selamat!", tanpa scroll & tanpa error console):
  ...
  - **Pasangan Pintar: 🧠 → 🤝** (keputusan pemilik di sesi yang sama). Otak mentah bukan gambar yang ramah untuk anak dan tak menggambarkan isi game; jabat tangan = "berpasangan", sesuai isinya (profesi↔alat, hewan↔rumah, lawan kata). **Penggantian ini baru benar-benar sampai ke layar intro pada 2026-08-09** — lihat entri di bawah.

- **Ikon game benar-benar SATU sumber: kartu portal & layar intro tak bisa beda lagi** (2026-08-09, laporan pemilik lewat tangkapan layar layar intro Pasangan Pintar: *"ini ganti dong gambarnya jadi yg menarik, jangan otak gini"* — 🧠 masih tampil di sana), teruji headless 380×800 & 360×640 (kartu & intro, game tetap bisa dimainkan, tanpa scroll horizontal & nol error console) + pemeriksaan menyeluruh **17 game: ikon kartu == ikon intro** (Jam Pintar dilewati, ikonnya muka jam SVG):
  - Sebabnya: ikon game ditulis di **DUA tempat** — `GameMeta.emoji` (`registry.ts`) dibaca kartu portal, `config.emoji` (config game) dibaca layar intro `GameShell`. Waktu ikon Pasangan Pintar diganti 2026-08-02, cuma registry yang diubah. Kartunya jadi 🤝, layar introhnya **tetap 🧠 selama seminggu**. Catatan lama sudah menulis "ikon dideklarasikan di SATU tempat", padahal yang benar-benar satu tempat baru `iconClock`.
  - Sekarang **registry MENANG**: `GamePage` mengoper `icon={meta.emoji}` ke `GameShell`, dan intro merender `icon ?? config.emoji`. `config.emoji` turun pangkat jadi **cadangan** (untuk shell yang dirender di luar portal) — jangan mengandalkannya, dan jangan mengira mengubahnya akan terlihat di app.
  - **Ganti ikon game = ubah `GameMeta.emoji` di `src/games/registry.ts`, satu baris.** Nilai di config boleh diselaraskan biar tak membingungkan, tapi bukan itu yang tampil.
  - **Ikon boleh berupa GAMBAR, bukan cuma emoji** (2026-09-02): `GameMeta.pic` menyebut file di `public/assets/games/<pic>.webp`, dirender `GameIcon` (`src/engine/ui/GameIcon.tsx`) di kartu portal dan `.game-big-pic` di layar intro — **satu sumber yang sama**, jadi tak bisa berbeda. `emoji` tetap wajib diisi: itu cadangan kalau file-nya gagal dimuat, dan identitas game di tempat lain. Sudah punya seni: Hutan Hewan (singa), Taman Huruf (tenda huruf), Labirin Warna (palet cat), Pasar Buah (semangka), dan sejak 2026-09-02 **seluruh sisa TK** — Kenal Huruf (huruf A pastel), Tulis Angka (pensil), Jalan Kendaraan (mobil merah), Kartu Kembar (sepasang kartu). Yang masih emoji tinggal kelompok SD1; tinggal menambah `pic` kalau seninya sudah ada.
    - Empat ikon TK itu datang sebagai JPEG Gemini berlatar kotak-kotak palsu, dipotong `python scripts/cut-checkerboard.py <art.jfif> public/assets/games/<id>.webp 320` — 320 px mengikuti empat ikon lama, dan hasilnya (21–44 kB) sudah dilihat di atas latar biru dulu: bagian terang milik gambar yang paling rawan terpotong (cincin perak pensil, kaca & lampu mobil, muka kartu yang krem) semuanya utuh.
  - Gambarnya diukur lewat **tinggi** saja (`width: auto`): tiap file dipotong pas di gambarnya, jadi rasionya beda-beda dan kotak berukuran tetap akan menggepengkan yang lebar.
  - Pola yang sama persis dengan `iconClock` (2026-08-02) — bedanya sekarang berlaku untuk ikon emoji biasa juga, jadi tak ada lagi jalur yang bisa menyimpang.
...
- **Lafal "e" taling: ejaan khusus mesin suara, bukan ejaan layar** (2026-08-08, laporan pemilik: *"pelafadzan bebek masih kurang general — lafadz e di awal beda dengan e di akhir"*), 70 baris di 10 game dirender ulang (3.942 karakter, 0,8% kuota); manifest tetap 723/723, nol entri tanpa file, kunci file TIDAK berubah:
  - Sebabnya sekelas dengan aturan angka di atas: bahasa Indonesia menulis **e pepet** (ə — "belas", "teman") dan **e taling** (é — "bébék", "méja") dengan huruf yang sama, jadi mesin suara harus MENEBAK per kata. Untuk "bebek" tebakan Azure meleset jadi "bəbek".
  - **`scripts/pronounce.mjs` = satu-satunya daftar**: kata → ejaan lafal, dipakai `render-narration.mjs` di `ssml()`. Hanya mengubah teks yang **dikirim ke Azure**; teks layar tetap ejaan resmi ("bebek") karena anak sedang belajar membaca dan tulisan Indonesia tidak memakai aksen. **Kunci manifest tetap teks layar**, jadi pemutar di app (`voice.ts`) tidak tahu-menahu soal lapisan ini dan tak perlu diubah.
  - **Tandai hanya suku kata yang taling**: `sepeda` → `sepéda`, BUKAN `sépéda`. Kata yang seluruhnya pepet tidak usah didaftarkan — itu tebakan bawaan Azure dan sudah benar.
  - **Kalau ragu, JANGAN didaftarkan.** "apel" & "wortel" sengaja dikeluarkan (banyak penutur mengucapkannya pepet); kata tak terdaftar = keadaan sekarang, jadi tak ada yang memburuk. Salah menandai justru merusak kata yang selama ini benar.
  - **Ejaan é, bukan tag `<phoneme>` IPA (keputusan pemilik).** Keduanya dirender jadi contoh dan pemilik menilai **bunyinya sama**, jadi dipilih yang paling murah dirawat: é cukup satu baris per kata dan otomatis ikut ke akhiran ("kelerengnya" lewat daftar `SUFFIX`), sedangkan IPA menuntut transkripsi manual tiap kata baru. Azure terbukti **menuruti keduanya** (ketiga varian menghasilkan audio berbeda) — jadi `<phoneme>` tetap tersedia kalau suatu saat ada kata yang é-nya tak mempan.
  - **Mengubah daftar lafal TIDAK memicu render ulang** — `key` berasal dari teks layar yang tak berubah, jadi file lama cuma dilewati. Paksa dengan **`npm run suara -- --redo-lafal`** (semua baris yang ejaan lafalnya beda dari teks layar) atau `--redo=<kata>` untuk satu kata. Di workflow: baris **`redo: lafal`** di `.github/render-request.txt`, atau centang `redo_lafal` di tombol Run workflow.
  - **Memilih dengan telinga itu bagian dari alurnya**: `scripts/sample-lafal.mjs` + `mode: lafal` di `.github/sample-request.txt` merender kalimat yang sama dalam varian sebelum/sesudah/IPA ke `sample-suara/` (di luar `public/`). Hapus foldernya setelah diputuskan.
  - **JEBAKAN: `sample-suara/` bisa hidup lagi sendiri.** Workflow contoh dipicu oleh PERUBAHAN `.github/sample-request.txt` — dan itu termasuk saat file yang sama ikut ter-merge ke `main`. Jadi menghapus folder contoh di branch fitur tidak cukup: begitu branch itu masuk `main`, workflow jalan lagi di `main` dan folder itu kembali. **Hapus folder contohnya SESUDAH merge**, dalam commit yang tidak menyentuh `sample-request.txt` (kalau menyentuhnya, terpicu lagi).
  - Panjang rekaman bergeser sampai ±3 detik di beberapa kalimat pendek, **median +0,02 detik** — suara HD memang tiap render sedikit berbeda, bukan tanda ada kata yang hilang.
...
- **Lagu kemenangan saat satu game tamat** (2026-08-08, permintaan pemilik *"setelah game selesai, berikan ringtone dong. Biar lebih menarik"*), teruji headless 380×800, 360×640 & 820×1180 — 26 pemeriksaan, nol error console:
  ...
  - `stop()` **menurunkan master gain 40 ms dulu** sebelum menghentikan osilator: memutus osilator di tengah gelombang berbunyi "klik" di speaker Android.
...
- **"Cerita Si Kancil" jadi "Cerita Anak" + 2 cerita baru** (2026-08-09, permintaan pemilik), teruji headless 380×800, 360×640 & 820×1180 (9 sesi dimainkan sampai "Selamat!", tiap pilihan yang kurang tepat ditekan lebih dulu, ketiga cerita muncul, tanpa scroll horizontal & nol error console):
  ...
  - Judul lama menutup pintu: game bernama "Cerita Si Kancil" tak bisa memuat cerita selain Kancil. Sekarang isinya tiga fabel — Kancil & Pak Tani (tetap, kalimatnya **tidak disentuh** supaya rekamannya terpakai lagi) + **Burung Jalak dan Kerbau** (saling menolong) + **Kancil dan Gajah** (akal + gotong royong).
  - **ISINYA FABEL HEWAN SAJA (keputusan pemilik).** Percobaan pertama mengisi dua slot baru dengan cerita keseharian anak — "Taman yang Bersih" (membuang sampah pada tempatnya) dan "Payung untuk Berdua" (Ayu berbagi payung dengan Beni) — dan **ditolak**: dua tokoh anak laki-laki & perempuan yang berjalan pulang di bawah satu payung terbaca seperti cerita romansa. Cerita baru di game ini harus bertokoh HEWAN. Keduanya dibuang seluruhnya (50 rekaman ikut di-prune), bukan ditambal kalimatnya.
  ...
  - Bedanya dengan Cerita Nusantara: di sana cerita rakyat & fabel klasik yang sudah dikenal (Timun Mas, Bawang Putih, Semut & Belalang); di sini fabel hewan Indonesia, **terutama Si Kancil**. Kalau menambah cerita rakyat baru, tempatnya Cerita Nusantara — jangan diadu. **(TIDAK BERLAKU LAGI sejak 2026-09-02: kedua game dilebur jadi "Baca Cerita" — lihat entri terbawah bagian ini.)**
  ...
  - **42 baris suara baru** (±2.100 karakter) di folder `cerita-kancil`, suara Ardi otomatis (skrip ekstraksi memilih Ardi untuk semua level `story-choice`). Ke-16 rekaman cerita Kancil yang lama tetap terpakai. Cerita yang dibuang meninggalkan 50 rekaman yatim — dibersihkan dengan menjalankan workflow render memakai **`prune: true`** (tombol Run workflow / `actions_run_trigger`), karena file pemicu `.github/render-request.txt` tidak punya baris untuk prune.
  - **Seni burung jalak + gambar item di halaman cerita** (2026-08-09, gambar dari pemilik), teruji headless 360×640, 380×800 & 820×1180 (`jalak.webp` & `elephant.webp` terambil 200, nol error console, tanpa scroll):
    - Item baru **`jalak`** (`public/assets/items/jalak.webp`, 461×512, 39 kB). Emoji 🐦 itu burung biru generik — jalak justru dikenali dari badan hitam, paruh & kaki kuning, dan garis putih di sayap; tak ada emoji yang membawanya.
    - **Fitur engine baru `StoryPage.item`** (id registry, dirender `ItemPic`, `emoji` tetap jadi cadangan): sebelum ini halaman cerita HANYA bisa emoji, jadi aturan lama "kalau soalnya memuat HEWAN pakai seni yang sudah diimpor" tak bisa dipatuhi di template `story-choice`. Sekalian gajah di "Kancil dan Gajah" memakai `elephant` yang sudah ada.
    - **Gambar halaman cerita sengaja LEBIH BESAR dari emoji yang digantikan** (`.story-emoji__img`, tinggi `clamp(170px, 50vw, 280px)` → 180px di HP 360): halaman tanpa pilihan isinya cuma gambar + satu-dua baris teks + tombol Lanjut, jadi layarnya lapang. **Halaman KEPUTUSAN tetap seukuran emoji** (`.story-emoji--choices`, `clamp(72px, 22vw, 140px)`) karena harus berbagi layar dengan dua-tiga kartu pilihan — aturan engine, bukan tambalan per-game. Terukur nol scroll di ketiga ukuran layar.
    - `max-width` + `object-fit: contain`, **jangan `height: auto`** — jebakan yang sama dengan `.choice-img`: hewan yang gambarnya lebar akan melebar keluar layar.
    - **`scripts/cut-item.py`** (baru): memotong SATU gambar kiriman pemilik jadi WebP transparan (`python3 scripts/cut-item.py <art.png> public/assets/items/<id>.webp`). Saudara dari `cut-item-sheet.py` yang untuk selembar berisi banyak item. Flood-fill dari tepi dengan toleransi tetangga 8, jadi **bagian putih yang TERKURUNG di dalam gambar tetap utuh** — garis putih di sayap jalak dan ujung ekornya justru bagian burungnya. Jangan pernah "membersihkan" sisa putih secara otomatis; lihat dulu hasilnya di atas latar berwarna.
    - Hewan lain di cerita-cerita ini masih emoji karena seninya belum ada: **kerbau, kancil, buaya, ular** (dan Pak Tani).
  - **JEBAKAN: halaman keputusan berpilihan TIGA cepat melebihi tinggi HP 360×640.** Percobaan pertama menaruh pertanyaan dua kalimat ("Di teras sekolah, Beni berdiri sendirian. Dia lupa membawa payung. Apa yang sebaiknya Ayu lakukan?") di atas tiga pilihan, salah satunya turun dua baris — terukur **+48px**, jadi pilihan terakhir ada di bawah lipatan. Halaman 3-pilihan Cerita Nusantara ("dompet") tidak begitu karena pertanyaannya satu kalimat pendek. Anggarannya: **pertanyaan ±50–60 karakter, pilihan ≤ 40 karakter dan hanya satu yang panjang**; konteks tambahan taruh di halaman SEBELUMNYA (halaman tanpa pilihan punya ruang lega). Jangan menambal dengan CSS — halaman 2-pilihan sudah muat, yang kepanjangan kalimatnya.

- **Anak memilih sendiri ceritanya: semua judul tampil di awal** (2026-08-09, permintaan pemilik *"6 judul langsung ditampilin diawal, biarkan anak memilih cerita yg mana"*), teruji headless 360×640, 380×800 & 820×1180 — 39 pemeriksaan: tiap cerita dimainkan sampai "Selamat!" (pilihan yang kurang tepat ditekan lebih dulu), kembali ke daftar judul, bintang tercatat di kartunya, tanpa scroll & nol error console:
  - Dulu engine yang mengundi: Cerita Nusantara mengambil 2 dari 6 cerita per sesi (`sessionLevels`), jadi anak yang ingin mengulang cerita kesukaannya harus menunggu undian dan tak pernah melihat judul yang lain ada. Buku cerita memang dipilih, bukan dibagikan.
  - **Fitur engine, bukan tambalan satu game**: `GameConfig.chooseLevel` (`{ title, again? }`) mengubah layar pembuka jadi **PEMILIH LEVEL**, dan `GameLevel.card` (`{ label, emoji?, item? }`) menentukan tampilan kartunya. Dipakai **Cerita Nusantara (6 judul) & Cerita Anak (3 judul)**; game lain tak berubah sedikit pun (layar intro + "Lanjut Level" masih diuji ulang di Hutan Hewan).
  - **`label` HARUS judul pendek** ("Timun Mas"), bukan `narration` — narasi itu kalimat lengkap yang diucapkan ("Timun Mas. Ayo bantu Timun Mas pulang dengan selamat!") dan tak muat di kartu.
  - **Syarat game ber-`chooseLevel`**: tiap slot satu level (bukan kolam varian) dengan `id` unik, karena kartunya menampilkan bintang per cerita. Cerita Nusantara karena itu jadi 6 slot; **id `l1`–`l3` sengaja tetap di cerita yang dulu jadi varian PERTAMA tiap slot** supaya bintang lama tidak hilang, tiga sisanya dapat `l4`–`l6`. Anak yang dulu mengumpulkan bintang lewat varian kedua akan melihat bintang itu di kartu varian pertama — tak terhindarkan saat satu id dipecah jadi tiga.
  - **NOL file suara baru** (manifest tetap 723 entri, `npm run narasi` tanpa perubahan): tombol 🔊 di tiap kartu memutar **`level.narration`** — kalimat yang memang sudah direkam. Kalau nanti mau kalimat ajakan ("Pilih ceritamu!") ikut dibacakan, itu baris baru yang harus dirender lewat workflow Azure dulu; sekarang ajakan itu teks layar saja.
  - **Pemilih = layar PEMBUKA, bukan di balik tombol "Mulai Main".** Permintaannya "langsung ditampilin diawal"; satu ketukan tambahan cuma untuk melihat judul-judul itu melawan intinya.
  - **JEBAKAN: tiap pilihan main di levelIndex 0 yang sama**, jadi tanpa `attemptKey` baru React memakai ulang komponen template yang lama — cerita kedua terbuka **tersangkut di halaman terakhir cerita sebelumnya**. `handlePick` menaikkan `attemptKey`; sudah diuji (memilih cerita lain sesudah tamat = halaman pertamanya sendiri).
  - **"Lanjutkan permainan" mati sendiri di mode ini**, dan memang tak ada yang bisa dilanjutkan: satu pilihan = satu level, jadi sesi tak pernah tersimpan (`saveSession` hanya jalan saat naik level). `getSession` sengaja tidak dibaca supaya sesi basi dari config lama tak melompat ke cerita acak.
  - Layar selesai menampilkan tombol `again` ("📚 Pilih Cerita Lain") yang kembali ke daftar judul, menggantikan "🔁 Main Lagi" — mengulang cerita yang sama tinggal satu ketukan dari sana.
  - Titik level di topbar disembunyikan kalau levelnya cuma satu (satu titik tak mengabarkan apa pun), tapi barisnya tetap ada supaya tombol ⬅️ dan 🔊 tidak bergeser.
  - **CSS: `.game-center--pick` pakai `justify-content: safe center`.** Dengan `center` biasa, konten yang lebih tinggi dari layar meluap ke ATAS dan tak bisa di-scroll. Browser Android lawas yang tak mengenal `safe` membuang barisnya dan jatuh ke flex-start — justru perilaku yang aman. Terukur muat satu layar tanpa scroll bahkan di 360×640 dengan 6 kartu.
...
- **ILUSTRASI ADEGAN: dua tokoh dalam satu gambar** (2026-08-10, permintaan pemilik dari tangkapan layar Cerita Anak *"Bisa ngga ini gambarnya kerbau dan jalak jadi 1 frame?"* + gambar kiriman pemilik), teruji headless 360×640, 380×800, 820×1180 & 740×380 (mendatar) — 52 pemeriksaan: cerita Jalak & Kerbau dimainkan sampai "Selamat!" (pilihan yang kurang tepat ditekan lebih dulu), tanpa scroll & nol error console:
  - Sebabnya: halaman cerita cuma bisa menampilkan SATU gambar item, jadi halaman yang kalimatnya menyebut dua tokoh ("burung jalak hinggap di punggung kerbau") memilih salah satunya saja. Anak kelas 1–2 belum lancar membaca dan **membaca adegannya dari gambar**, jadi tokoh yang tak digambar hilang dari ceritanya.
  - **PERCOBAAN PERTAMA DITOLAK PEMILIK ("jelek"): menempelkan dua gambar terpisah berdampingan.** Waktu itu jalak (seni WebP) dijejerkan dengan kerbau (emoji 🐃, seninya belum ada) dalam satu baris. Pelajarannya bukan soal emoji-vs-seni saja — **dua tokoh yang BERINTERAKSI tidak bisa dirakit dari dua gambar terpisah**: burung yang seharusnya bertengger di punggung malah berdiri di sebelahnya, dan tak ada penempatan yang membuatnya benar tanpa mengetahui bentuk gambar kerbaunya. Jangan diulang; minta satu ilustrasi utuh.
  - **Fitur engine `StoryPage.art`**: nama file di `public/assets/story/<nama>.webp` — ilustrasi yang **membawa latarnya sendiri**, dirender sebagai panel lebar bersudut membulat (`.story-art`). Bedanya dengan `item`: item = satu benda/hewan yang dipotong transparan dan boleh dipakai di soal mana pun; ilustrasi adegan = satu momen cerita, tak bisa dipakai ulang di tempat lain. `item`/`emoji` halaman itu tetap jadi cadangan kalau filenya gagal dimuat (deploy setengah jadi) — halaman cerita tidak boleh kosong.
  - **Latar engine (`Scene`) TETAP digambar di belakang panelnya.** Sempat dicoba dimatikan (ilustrasinya kan sudah punya latar) dan hasilnya justru jelek: yang mengelilingi panel jadi gradien pastel bawaan app, bertabrakan dengan lukisan yang hijau-biru. Dengan latar `sawah` tetap hidup, sekelilingnya langit + rumput dan panelnya menyatu. **Pilih `scene` yang tempatnya sama dengan ilustrasinya.**
  - **`width: 100%` + `height: auto` di sini BENAR** — kebalikan dari aturan `.choice-img`/`.road-marker-img` (yang mengotakkan gambar pengganti emoji jadi persegi). Yang dijaga di sini adalah rasio lukisannya; lebarnya sudah dibatasi kolom `.game-area`. `max-height` (46vh, 28vh di halaman keputusan, 34vh/20vh saat layar mendatar) yang menjaga halaman tetap muat.
  - **`scripts/story-art.py`** (baru): memperkecil & mengompres ilustrasi kiriman pemilik ke WebP — **tanpa flood-fill sama sekali**, karena latarnya justru dipakai. Bawaannya lebar 900px (`jalak-kerbau.webp` = 47 kB); di layar paling lebar ±340px CSS, jadi 900px sudah tajam untuk layar 2–3×.
  - **HALAMAN KEPUTUSAN JUGA BOLEH BERILUSTRASI** (permintaan pemilik di sesi yang sama, dari tangkapan layar halaman "Apa yang sebaiknya kerbau jawab?" yang cuma menampilkan 🤔). Emoji berpikir itu isyarat "ayo pilih"; kalau adegannya punya ilustrasi, ilustrasi itu yang harus tampil — anak menimbang pilihannya sambil melihat kejadiannya. `ask()` di `cerita-kancil.ts` karena itu menerima `'🤔'` ATAU `{ art, emoji }`.
  - **Ilustrasi di cerita Jalak & Kerbau — satu adegan satu gambar**: `jalak-kerbau.webp` ("bolehkah aku hinggap"), `jalak-kerbau-jawab.webp` (halaman keputusan), `jalak-kerbau-kutu.webp` (jalak menunduk mematuki kutu) — dan sejak 2026-09-03 empat lagi: `-kubang` (pembuka), `-ular` (keputusan kedua), `-lari`, `-sahabat` (penutup), sehingga cerita ini bergambar penuh (lihat entri terbawah bagian ini). Pemilik mengirimnya bertahap dan tiap kali memakai gambar yang sama di dua halaman terasa kurang: gambar halaman cerita itu MENERANGKAN kalimatnya, jadi kalimat yang berbeda pantas dapat gambar sendiri. Menambah adegan berikutnya = satu file + satu nama di config.
  - **JEBAKAN: `width: 100%` + `object-fit: contain` membuat sudut membulatnya salah.** Di halaman keputusan panelnya dibatasi TINGGI, jadi dengan `width: 100%` kotak elemennya tetap selebar kolom sementara gambarnya menciut di dalam — `border-radius` memotong ruang kosong dan di layar gambarnya tampak bersudut siku. Yang benar: `width`/`height` `auto` + `max-width`/`max-height`, biar kotaknya mengikuti rasio gambarnya sendiri.
...
- **"Cerita Anak" + "Cerita Nusantara" DILEBUR jadi satu game "Baca Cerita"** (2026-09-02, permintaan pemilik: *"Jadikan 1 aja, dengan judul baca cerita. Gambar icon nya pake yg cerita anak"*), typecheck + build lulus, `npm run narasi` bersih:
  - **9 cerita dalam satu game**, anak tetap memilih judulnya sendiri. Ikon kartu memakai milik Cerita Anak: emoji 📗 + gambar `public/assets/games/cerita-kancil.webp`. Ikon `cerita-nusantara.webp` DIBIARKAN di repo — tidak lagi dipakai, tapi menghapus aset kiriman pemilik bukan keputusan yang boleh diambil sambil lalu.
  - **Id game tetap `cerita-kancil`** — ini penggantian nama KETIGA ("Cerita Si Kancil" → "Cerita Anak" → "Baca Cerita") dan id-nya tak pernah ikut berubah, karena dipakai route `/game/:id`, bintang anak, dan folder file suaranya. Nama file yang tak lagi cocok dengan judulnya (`src/games/sd1/cerita-kancil.ts`) itu harga yang sengaja dibayar.
  - **Id level cerita pindahan jadi `n1`–`n6`, bukan `l4`–`l9`.** `l1`–`l3` sudah dipakai fabel Kancil jadi harus digeser bagaimanapun; awalan `n` membuat asalnya terbaca DAN membuat pemindahan bintangnya cuma soal menukar awalan. Nomornya sengaja sama dengan id lamanya di Cerita Nusantara.
  - **Bintang lama dipindahkan, bukan dibiarkan hangus**: `migrateMergedStories()` di `progress.ts`, dipanggil di `main.tsx` SEBELUM React menggambar. Tanpa itu anak yang sudah menamatkan Timun Mas kehilangan bintangnya **dan maskotnya ikut mengecil**, karena `getTotalStars()` yang menumbuhkannya. Kunci `cerita-nusantara` DIHAPUS setelah dipindah — kalau ditinggal, bintangnya terhitung dua kali dan maskot melonjak tanpa sebab. Aman dijalankan berulang.
  - **NOL render ulang suara.** Kunci file = hash isi kalimat, dan tak ada satu kalimat pun yang diubah; yang berubah cuma FOLDER-nya, karena folder ditentukan game yang mengucapkannya. Jadi 108 mp3 dipindah `voice/cerita-nusantara/` → `voice/cerita-kancil/` (jadi 166 file) dan 108 entri `manifest.json` diubah awalannya. **Kalau kalimatnya ikut diubah, biayanya bukan memindah file lagi tapi merender ulang.**
  - Aturan lama *"cerita rakyat baru tempatnya Cerita Nusantara"* SUDAH TIDAK BERLAKU — sekarang semua ke `cerita-kancil.ts`. Yang masih berlaku: **cerita buatan sendiri harus bertokoh HEWAN** (lihat entri 2026-08-09 soal payung yang terbaca romansa). Cerita rakyat yang sudah dikenal boleh bertokoh manusia — Timun Mas & Bawang Putih tetap di sini karena anak dan orang tuanya sudah mengenal jalan ceritanya.
  - **BUG LAMA yang ikut diperbaiki: `npm run narasi` tak pernah bisa jalan di Windows.** `scripts/extract-narration.mjs` menanam `path.resolve(f)` mentah ke dalam string import; di Windows itu `C:\Users\...` dan backslash-nya dimakan sebagai escape, esbuild lalu gagal `Could not resolve` untuk ke-18 config. Sekarang `path.resolve(f).split(path.sep).join('/')`. **`pathToFileURL()` BUKAN jawabannya** — sudah dicoba, esbuild tidak menerima `file://` sebagai specifier. Di Linux (CI) penggantian ini tidak mengubah apa pun.

- **Baca Cerita: 3 cerita aktif, 6 sisanya "segera hadir"** (2026-09-03, permintaan pemilik: *"dari 9 cerita bikin 3 dulu yang aktive salah satunya jalak dan kerbau. sisanya jadikan disable dulu"* — selagi tampilan ceritanya dipercantik), typecheck + build lulus:
  - **Yang aktif `l1`-`l3`: Kancil dan Pak Tani, Jalak dan Kerbau, Kancil dan Gajah.** Pemilik menyebut Jalak dan Kerbau; tiga fabel Kancil itu satu kelompok yang sudah berdampingan di urutan daftar, dan `l2`/`l3` yang punya ilustrasi kiriman pemilik — jadi mereka yang dibuka. Enam cerita rakyat `n1`-`n6` dimatikan.
  - **Mekanismenya `card.soon` di `LevelCard` (`types.ts`), BUKAN mengeluarkan level dari `levels`.** Kalau levelnya dibuang, id-nya lenyap dari config dan **bintang yang sudah dikumpulkan anak untuk 6 cerita itu ikut hilang** — risiko yang sama persis dengan yang dijaga saat peleburan 2026-09-02. Dengan `soon`, isinya utuh di tempatnya dan membukanya lagi = menghapus satu baris `soon: true`.
  - **Kartunya mati TOTAL, bukan sekadar redup**: `<button disabled>` + `.pick-card--soon` (garis putus-putus, tanpa bayangan terangkat, isi opacity 0,42), dan **tombol 🔊-nya diganti 🔒 di sudut yang sama**. Kalau 🔊 dibiarkan, kartunya tetap menjawab saat disentuh padahal ceritanya tidak bisa dibuka — anak menyimpulkan game-nya rusak, bukan "belum ada". `.btn:active` ikut dimatikan: di WebView Android tombol disabled masih bisa kena `:active` dan kartunya turun 4px seolah tertekan.
  - **`npm run narasi` sengaja TIDAK dijalankan ulang dan mp3 enam cerita itu tidak dihapus.** Tak ada satu kalimat pun yang berubah; menghapus rekamannya berarti harus merender ulang saat kartunya dibuka lagi.

- **"Jalak dan Kerbau" jadi cerita PERTAMA yang bergambar penuh** (2026-09-03, empat gambar kiriman pemilik bertahap dalam satu sesi: `jalak-kerbau-kubang`, `-ular`, `-lari`, `-sahabat`), typecheck + build lulus, keempat WebP ikut ter-copy ke `dist/assets/story/`:
  - Sesudah tiga ilustrasi 2026-08-10 tersisa empat halaman beremoji polos — keempatnya sekarang bergambar, jadi **ketujuh halaman cerita ini berilustrasi tanpa satu pun emoji tersisa**: berkubang → jalak minta izin → keputusan "apa yang sebaiknya kerbau jawab" → mematuki kutu → keputusan ular → kerbau lari → penutup bersahabat. Emoji lamanya (🐃 🐦 🤔 🐍 🏃 ⭐) tetap tertulis sebagai cadangan kalau filenya gagal dimuat.
  - **Ilustrasi penutupnya bermandi cahaya SENJA** sementara latar engine di belakangnya (`sawah`) tetap siang. Dibiarkan: `malam` — satu-satunya latar bernuansa senja — langitnya lilac kebiruan dengan bulan sabit & bintang, jadi justru lebih bertabrakan dengan lukisan jingga itu daripada hijau sawah yang tempatnya memang sama. Alasannya ditulis juga di config. Kalau nanti mau pas betul, yang dibutuhkan latar engine baru (`senja`), bukan menukar `sawah` ke `malam`.
  - **Halaman ular itu halaman KEPUTUSAN**, jadi panelnya dibatasi 28vh (`.story-art--choices`) supaya tetap muat bersama dua kartu pilihan — aturan engine yang sudah ada, tak ada CSS baru. Gambarnya juga paling lebar di cerita ini (900×502; yang lain 900×672), jadi di layar justru paling pendek.
  - **Ularnya digambar tersenyum ramah**, padahal kalimatnya "ular besar mendekat" dan itu momen bahaya. Dibiarkan apa adanya (gambar kiriman pemilik, dan gaya buku anak memang begitu), tapi dicatat: anak membaca adegan dari gambarnya, jadi kalau suatu saat halaman ini terasa tidak menegangkan, ular inilah sebabnya — bukan kalimatnya.
  - **`at('sawah')` SENGAJA TETAP di halaman pertama** (dan berlaku sampai penutup) — ini kebalikan dari aturan "halaman berilustrasi tidak memakai `at()`" yang dipakai di "Kancil dan Gajah", dan bedanya disengaja: di sana latar engine yang tersedia (`sungai`) bukan tempat yang digambar (rawa), di sini `sawah` SEPADAN dengan lukisannya. Mematikan latarnya di cerita ini sudah pernah dicoba 2026-08-10 lalu dibatalkan — sekeliling panel jadi gradien pastel bawaan app yang bertabrakan dengan lukisan hijau-biru. Alasannya ditulis juga sebagai komentar di config, karena membaca dua aturan yang tampak bertentangan tanpa penjelasan itu jebakan.
  - **Kalimatnya TIDAK disentuh sama sekali**, jadi `npm run narasi` tetap 770 baris (terverifikasi: `scripts/narration-lines.json` nol perubahan) dan rekaman lamanya terpakai — kunci file suara = hash isi kalimat. Menambah gambar tidak boleh sekalian "merapikan" kalimat.
  - **Burung jalaknya sudah bertengger di punggung kerbau** di gambar `kubang`, padahal ia baru meminta izin di halaman berikutnya. Dibiarkan: `jalak-kerbau.webp` (halaman "bolehkah aku hinggap") juga sudah menggambarkannya di punggung, jadi urutannya konsisten dengan yang sudah dipakai anak selama ini. Kalau nanti dirasa mengganggu, gambar ini pindah ke halaman penutup ("Sejak itu jalak dan kerbau selalu bersama") — satu baris di config, filenya tidak berubah.
  - **Ukuran** (`scripts/story-art.py`, lebar 900): `kubang` 79 kB, `ular` 69 kB, `lari` 64 kB, `sahabat` 58 kB — lebih berat dari tiga ilustrasi lama cerita ini (41–50 kB) karena sawah, gunung, dan pohonnya jauh lebih ramai. Masih di bawah `kancil-tani-lapar.webp` (97 kB); jangan diperbesar lagi.
  - **BELUM diuji headless**: sesi ini tak punya browser. Yang sudah dipastikan cuma typecheck + build + berkasnya terbit di `dist/`.

- **"Kancil dan Gajah" ikut bergambar — enam dari tujuh halaman** (2026-09-03, empat gambar kiriman pemilik di sesi yang sama dengan Jalak & Kerbau: `kancil-gajah-tanya`, `-panggil`, `-ranting`, `-tarik`), typecheck + build lulus, keempat WebP ikut ter-copy ke `dist/assets/story/`:
  - Dua halaman pembukanya sudah berilustrasi sejak 2026-08-10 (`kancil-rawa`, `gajah-lumpur` — lewat `StoryPage.item`, lihat entri di atas). Empat halaman berikutnya yang tadinya emoji polos sekarang bergambar lewat `StoryPage.art`: keputusan "gajah terlalu berat" (kancil berhadapan dengan gajah yang terperosok), kancil memanggil teman-temannya dari atas batu, keputusan "lumpurnya licin" (tumpukan ranting & daun pisang di tepi lumpur), lalu semua menarik rotan bersama. Emoji lamanya (🤔 🐃 🌿 💪) tetap tertulis sebagai cadangan.
  - **Halaman penutup (⭐ "Badan kecil pun bisa menolong") SENGAJA dibiarkan beremoji**: kalimatnya pesan moral, bukan adegan — satu-satunya gambar yang cocok adalah mengulang gambar halaman sebelumnya, dan itu justru melemahkan keduanya. Jadi cerita ini **bukan** "bergambar penuh" seperti Jalak & Kerbau; jangan dicatat begitu.
  - **`at('sungai')` DIGESER dari halaman keputusan pertama ke halaman penutup**, persis mengikuti aturan yang sudah ditulis di entri 2026-08-10 cerita ini: halaman berilustrasi tidak memakai `at()` (gambarnya sudah membawa rawanya sendiri), dan latar engine dipakai di halaman pertama yang BELUM berilustrasi — yang sekarang tinggal penutupnya. Menggeser ke bawah aman karena halaman tanpa `scene` mewarisi latar halaman SEBELUMNYA; menghapus `at()` sama sekali tidak, penutupnya akan jatuh ke gradien pastel bawaan app.
  - **Aturannya kebalikan dari Jalak & Kerbau dan itu memang disengaja** (kedua cerita disentuh di sesi yang sama, jadi mudah terbaca sebagai tidak konsisten): di Jalak latar engine `sawah` SEPADAN dengan lukisannya, di sini latar yang tersedia (`sungai`) bukan tempat yang digambar (rawa). Latar `rawa` sendiri belum ada — kalau nanti dibuat, halaman-halaman cerita ini yang pertama memakainya.
  - **Keempat gambarnya satu latar dengan dua ilustrasi lama** (rawa berteratai, tepian lumpur, hutan berkabut) — permintaan yang ditulis di entri 2026-08-10 ("minta pemilik memakai latar yang sama supaya halamannya terbaca satu perjalanan") ternyata dipenuhi. Rusa, kerbau, monyet & burung enggangnya juga tokoh yang sama di keempat gambar.
  - **Dua halaman keputusan dapat gambar yang MENDATAR** (900×502; dua halaman biasa 900×672) — kebetulan yang menguntungkan: panel halaman keputusan dibatasi 28vh (`.story-art--choices`) supaya muat bersama kartu pilihan, jadi gambar yang lebih pendek justru paling aman. Tak ada CSS baru.
  - **Ukuran** (`scripts/story-art.py`, lebar 900): `tanya` 50 kB, `ranting` 63 kB, `tarik` 80 kB, `panggil` 82 kB — setara set Jalak (58–79 kB) dan masih di bawah `kancil-tani-lapar.webp` (97 kB). Jangan diperbesar lagi.
  - **Kalimatnya TIDAK disentuh sama sekali**, jadi `npm run narasi` tetap 770 baris (terverifikasi: `scripts/narration-lines.json` nol perubahan setelah dijalankan ulang) dan rekaman lamanya terpakai — kunci file suara = hash isi kalimat.
  - **BELUM diuji headless**: sesi ini tak punya browser. Yang sudah dipastikan cuma typecheck + build + berkasnya terbit di `dist/`.

- **Layar `ErrorBoundary` pakai gambar naga, bukan emoji 🐣** (2026-09-03, pemilik menemui layar itu di HP lalu: *"mending diganti gambar iconnya"*), typecheck + build lulus:
  - **Kenapa layarnya muncul sama sekali** (didiagnosis dulu sebelum menyentuh gambarnya): hari itu ada **5 deploy** ke branch Pages, terakhir 13:08. Tiap build mengganti hash **semua** chunk dan menghapus yang lama (terverifikasi: 22 file `R0xx` antara dua commit build terakhir). HP yang app-nya sudah terbuka SEBELUM deploy lalu membuka halaman/game — semuanya `lazy()` — meminta nama file lama → 404 → import gagal → `ErrorBoundary`. Bukti pendukung di tangkapan layarnya: tombol & font sudah bergaya, artinya JS+CSS utama sudah termuat duluan. **Obatnya tombol "Coba Lagi" yang memang sudah ada di situ.** Kalau layarnya muncul lagi di tempat yang sama SESUDAH reload, itu bug asli, bukan sisa deploy — jangan tertukar.
  - Gambar: `public/assets/ui/tersendat.webp` (237×384, 19 kB) — naga maskot menggaruk kepala, senyum malu-malu, satu balok mainan terguling. **Folder `public/assets/ui/` baru**, untuk seni layar sistem yang bukan game/maskot/cerita.
  - **Emoji 🐣 tetap jadi cadangan** lewat state `artFailed` + `onError` — kontrak yang sama dengan `MascotPic`/`FeedbackPic`, dan di sini paling penting: layar ini muncul PERSIS saat sesuatu sudah rusak, jadi gambarnya tidak boleh jadi penyebab layar kosong berikutnya. Class component, jadi pakai `this.setState`, bukan `useState`.
  - **Kenapa 🐣 diganti**: emoji itu **maskot tahap 2** (`Si Menetas`, 10 ⭐ di `mascot.ts`) — di layar error ia terbaca seolah berhubungan dengan tahap maskot anak. Ditambah alasan lama: emoji digambar berbeda di tiap HP.
  - Ukurannya dibatasi `maxHeight: '40vh'` selain `width: 180`: gambarnya menjulang (rasio 0,62), dan di layar 360×640 tanpa batas itu judul + tombolnya terdesak.
  - Prompt gambarnya (dua pilihan + versi Inggris + aturan "jangan bahasa gambar error, jangan yang menangis"): **`docs/prompt-gambar-tersendat.md`**.
  - **`scripts/cut-soft.py` (BARU)** — pemotong latar untuk seni **render 3D lembut tanpa outline** (maskot). `cut-item.py` merusak gambar ini: rantai toleransinya menuruni landaian tepi yang halus, masuk lewat celah sempit antara lengan dan badan, lalu **melahap perut krem naganya** — dan angka yang dicetaknya tetap terlihat wajar ("latar terbuang 78,6%"), jadi kerusakannya hanya ketahuan setelah ditempel di atas latar berwarna. Yang baru memakai ambang global + **erode → telusur dari tepi → dilate**, jadi jembatan sempit putus dan kebocoran mustahil; karena aman itu ambangnya boleh longgar (205) sehingga **bayangan lantai** yang pucat ikut terbuang, dan alpha-nya diberi **feather 1px** supaya siluet tanpa outline tidak bergerigi di 180px. **Pakai skrip ini juga untuk maskot tahap 10–12.**
  - **BELUM diuji headless**: sesi ini tak punya browser. Yang dipastikan: typecheck, build, hasil potong ditempel di atas latar berwarna & di atas gradien app pada ukuran tampil sesungguhnya.
...
- **Di Windows, set variabelnya lewat PowerShell**, bukan Git Bash: `$env:DEPLOY_BASE='/Game-Edukasi-Anak/app/'; $env:VITE_USE_HASH_ROUTER='1'; npm run build`. Git Bash (MSYS) menerjemahkan nilai yang berbentuk path Unix jadi path Windows — `DEPLOY_BASE=/Game-Edukasi-Anak/app/` diam-diam berubah jadi `/Program Files/Git/Game-Edukasi-Anak/app/`, dan **build-nya tetap sukses**, jadi salahnya baru ketahuan kalau `dist/index.html` diperiksa. Selalu cek `grep -o 'src="[^"]*index-[^"]*"' dist/index.html` sebelum menyalin ke branch Pages (2026-09-02).
