...
- **Dua tokoh dalam SATU frame halaman cerita** (2026-08-10, permintaan pemilik dari tangkapan layar Cerita Anak: *"Bisa ngga ini gambarnya kerbau dan jalak jadi 1 frame?"*), teruji headless 360×640, 380×800 & 820×1180 — 39 pemeriksaan: cerita Jalak & Kerbau dimainkan sampai "Selamat!" (pilihan yang kurang tepat ditekan lebih dulu), tanpa scroll & nol error console:
  - Sebabnya: halaman cerita cuma bisa menampilkan SATU gambar, jadi halaman yang kalimatnya menyebut dua tokoh ("burung jalak hinggap di punggung kerbau") memilih salah satunya saja — kerbau di satu halaman, jalak di halaman berikutnya. Anak kelas 1–2 belum lancar membaca dan **membaca adegannya dari gambar**, jadi tokoh yang tidak digambar praktis hilang dari cerita.
  - **Fitur engine, bukan tambalan satu game**: `StoryPage.cast` = daftar `StoryFigure { item?, emoji?, size? }`, dirender berjajar dalam satu frame (`.story-cast` di `engine.css`). Kalau `cast` diisi, `item`/`emoji` halaman itu diabaikan. Halaman satu tokoh tidak berubah sedikit pun.
  - **Batas wajar 2–3 tokoh.** Tinggi frame `cast` sengaja lebih pendek dari gambar tunggal (`clamp(140px, 40vw, 230px)` vs `clamp(170px, 50vw, 280px)`): tokohnya berbagi lebar layar, dan frame setinggi gambar tunggal membuat halaman ikut scroll di HP 360×640. Halaman KEPUTUSAN mengecil seperti biasa (aturan `.story-emoji--choices` yang sudah ada).
  - **`size: 'kecil'` (setengah tinggi) itu bagian dari ceritanya**, bukan urusan tata letak: burung sebesar kerbau akan salah mengajarkan. Tiap tokoh diukur dari satu variabel CSS `--fig` supaya gambar & emoji sebanding.
  - **Emoji dapat `margin-bottom` negatif (−0,12 × `--fig`)**: kotak emoji menyisakan ruang turunan di bawah garis dasar, jadi tanpa itu tokoh emoji tampak melayang di samping tokoh bergambar yang berdiri di garis yang sama. Terukur selisih alas 12–18px (dari 168–267px tinggi tokoh).
  - **Kerbau masih emoji 🐃 karena seninya belum ada** — sengaja BUKAN `cow` (itu sapi; satu gambar tidak boleh punya dua arti). Begitu seni kerbau dikirim pemilik: potong dengan `scripts/cut-item.py`, daftarkan id `kerbau` di `items.ts`, lalu ubah `KERBAU` di `cerita-kancil.ts` dari `{ emoji: '🐃' }` jadi `{ item: 'kerbau' }` — tak ada yang lain yang berubah.
  - **Burung belum bisa BERTENGGER di punggung kerbau, masih berdampingan.** Menumpuk dua gambar terpisah butuh titik tempel yang pas untuk gambar kerbau tertentu; selama kerbaunya masih emoji (bentuknya beda di tiap HP) titik itu tak bisa ditentukan. Kalau nanti seni kerbau sudah ada dan pemilik mau versi bertengger, itu satu field tambahan di `StoryFigure` yang dikalibrasi ke gambar itu.
