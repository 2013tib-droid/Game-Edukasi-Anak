...
- **ILUSTRASI ADEGAN: dua tokoh dalam satu gambar** (2026-08-10, permintaan pemilik dari tangkapan layar Cerita Anak *"Bisa ngga ini gambarnya kerbau dan jalak jadi 1 frame?"* + gambar kiriman pemilik), teruji headless 360×640, 380×800, 820×1180 & 740×380 (mendatar) — 52 pemeriksaan: cerita Jalak & Kerbau dimainkan sampai "Selamat!" (pilihan yang kurang tepat ditekan lebih dulu), tanpa scroll & nol error console:
  ...
  - **`width: 100%` + `height: auto` di sini BENAR** — kebalikan dari aturan `.choice-img`/`.road-marker-img` (yang mengotakkan gambar pengganti emoji jadi persegi). Yang dijaga di sini adalah rasio lukisannya; lebarnya sudah dibatasi kolom `.game-area`. `max-height` (46vh, 28vh di halaman keputusan, 34vh/22vh saat layar mendatar) yang menjaga halaman tetap muat.
  ...
  - **Satu ilustrasi dipakai di DUA halaman** cerita Jalak & Kerbau ("bolehkah aku hinggap" dan "mematuki kutu"). Kalau nanti ada ilustrasi kedua untuk momen "meminta izin" (jalak masih di pagar), tinggal tambah filenya + ganti satu nama di config.
  - **Layar MENDATAR memang masih scroll di halaman keputusan (81px)** — itu bug lama, terukur sama persis sebelum perubahan ini. Halaman bergambarnya justru membaik: dulu 88px scroll, sekarang 0.
